...
class DayReview(ctk.CTkFrame):
    def __init__(self, parent, main_app, plan_name, day, save_progress):
        ...
        plan_key_map = {
            "Plan Ușor 1": "easy_plan_1",
            "Plan Ușor 2": "easy_plan_2",
            "Plan Mediu 1": "medium_plan_1",
            "Plan Mediu 2": "medium_plan_2",
            "Plan Intens 1": "hard_plan_1",
            "Plan Intens 2": "hard_plan_2",
        }
        plan_key = plan_key_map.get(plan_name, plan_name.lower().replace(" ", "_"))
        ...
        exercise_list = exercises_dict.get(plan_key, {}).get(self.day, [("Rest Day", "")])
    ...
    def start_workout(self):
        ...
        if not user_data:
            print("Eroare: Utilizatorul logat nu exista in mongo")
            return

        user_id = str(user_data["_id"])

        plan_map = {
            "Plan Ușor 1": "Easy Plan 1",
            "Plan Ușor 2": "Easy Plan 2",
            "Plan Mediu 1": "Medium Plan 1",
            "Plan Mediu 2": "Medium Plan 2",
            "Plan Intens 1": "Hard Plan 1",
            "Plan Intens 2": "Hard Plan 2",
        }
        plan_key = plan_map.get(self.plan_name, self.plan_name)
        video_path = f"home_plan_workouts/tutorials/{plan_key.lower().replace(' ', '')}_day{self.day}.mp4"
